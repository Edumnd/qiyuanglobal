...
function ttResetSize(e) {
	tt_view_w=YAHOO.util.Dom.getViewportWidth();
	tt_view_h=YAHOO.util.Dom.getViewportHeight();
}

function ttResetScroll(e) {
	screenPos={x:YAHOO.util.Dom.getDocumentScrollLeft(),y:YAHOO.util.Dom.getDocumentScrollTop()};
}
...
function init_tt() {
	...
	YAHOO.util.Event.addListener ( window, 'resize', ttResetSize );
	YAHOO.util.Event.addListener ( window, 'scroll', ttResetScroll );
	ttResetSize();
	ttResetScroll();
}

YAHOO.util.Event.onDOMReady( init_tt );
...
function showTip(e,elem) {
	...
	if (tt_curElem != elem) {
		...
		if (tt_curElem) {
			...
			//_moveNodes(YAHOO.util.Dom.get(tt_curElem.tt_div),tt_inner);
            _moveNodes(tt_curElem.tt_div,tt_inner);
		...
		//_moveNodes ( tt_inner, YAHOO.util.Dom.get(elem.tt_div) );
        _moveNodes ( tt_inner, elem.tt_div );
		tt_div.appendChild ( tt_inner );
	...
	var r=YAHOO.util.Dom.getRegion('yh_tt');	
...
function createTip ( elem ) {
    ...
    //elem.tt_div= new YAHOO.util.Element(elem.title);
    elem.tt_div = document.createElement('div');
    elem.tt_div.innerHTML = elem.title;
    elem.title = '';
    
	elem.tt_noheader= true;//!arguments[2];
    ...
    */
    ...
    YAHOO.util.Event.addListener ( elem, 'mouseover', showTip, elem );
    YAHOO.util.Event.addListener ( elem, 'mouseout', hideTip );
    YAHOO.util.Event.addListener ( elem, 'mousemove', showTip, elem );
